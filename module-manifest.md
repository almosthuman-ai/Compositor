...
| Native editor | `windows/compositor/ui.py` | Qt canvas, pointer painting and selections, layer controls, menus, generation candidates, ChatGPT panel, optional embedded Studio view. |
...
The document and native UI integration has passed 38 focused tests on Windows 10. The installed GUI and packaged MCP have been exercised together: document creation, editing, stale-revision rejection, actual image inspection, layered save, exact export pixels and undo. `windows/verify_distribution.py` repeats this journey in an isolated store and runs after the CI build.
