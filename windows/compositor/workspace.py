...
class Workspace(QObject):
    ...
    def state(self):
        return {'service':'compositor','activeDocument':self.active,'documents':[d.info() for d in self.documents.values()],
                'view':self.view,'jobs':list(self.generation.jobs.values()),'settings':self.settings.public()}
    ...
    def dispatch(self,action,args=None):
        ...
        if action=='activate':
            self.active=self.document(a['documentId']).id; self.changed.emit(); return self.document().info()
        if action=='view':
    ...
    def save_recovery(self):
        ...
        try:
            ...
            atomic_json(folder/'session.json',{'documents':records,'active':self.active})
