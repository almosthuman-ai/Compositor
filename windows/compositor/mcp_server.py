...
def compositor_glitch(operation:Literal['catalog','render','export','vary','normalize','palette','jobs','job','apply','cancel']='catalog',args:dict|None=None) -> dict|list:
    """Use Tai Mei's Glitch Temple in the shared editor, without desktop focus. catalog returns all 18 effects, parameter ranges, defaults and complete templates. Start with defaults, choose effects from templates, retain their where targets and optional character/sort/wizprocess settings. render args: documentId, source (layer/composite/generated/treatment), layerId, recipe, name, preserveAlpha, useSelection, autoApply. Rendering is asynchronous: inspect job with jobId, then inspect its actual image and apply. Layer treatments retain the original source in the document for later source=treatment revisions. Structure/color seeds are retained; source document changes reject automatic placement. export renders an applied treatment as a GIF or MP4 within the complete composition, preserving other layers: args documentId, layerId, path (new filename), frames (2-240), fps (1-30), maxDimension (default 960 for ordinary images), or scale (integer 1-16 for pixel art, default 1). Pixel scale preserves complete logical pixels and cannot be combined with maxDimension; output is limited to 3840 pixels per side. Poll job for framesRendered/status/exportPath; cancel stops the selected job. MP4 pads odd dimensions to even with black; GIF retains transparency. Pixel documents default to recipe.compositor.pixelFinish={paletteMode: document, hardAlpha: true, dither: false}; paletteMode=effect retains effect colors. Finishing applies to stills and every animation frame; document palettes are captured in the applied recipe. vary uses variation=structure or colors. Rendering uses the bundled Canvas engine, not Processing."""
    return call('glitch',{'operation':operation,**(args or {})})

@server.tool(annotations=ToolAnnotations(readOnlyHint=True,destructiveHint=False))
def compositor_inspect_glitch(jobId:str,maxDimension:int=1600,scope:Literal['composition','layer','original']='composition') -> list:
    """See the captured composition with the actual Glitch Temple candidate, the isolated treatment layer, or the original composition. Preview and application share layer preparation; other layers and masks remain in context."""
    result=call('glitch',{'operation':'capture','jobId':jobId,'maxDimension':maxDimension,'scope':scope})
