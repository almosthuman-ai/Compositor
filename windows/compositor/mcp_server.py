...
@server.tool()
def compositor_prepare_generation(kind:Literal['generate','edit','patch']='edit',documentId:str|None=None,box:dict|None=None,prompt:str='',styleId:str|None=None,purpose:Literal['image','character']='image',characterId:str|None=None,resampling:Literal['smooth','nearest']|None=None) -> dict:
...
@server.tool()
def compositor_generate(prompt:str,kind:Literal['generate','edit','patch']='generate',documentId:str|None=None,provider:str|None=None,model:str|None=None,box:dict|None=None,references:list|None=None,size:str='1024x1024',autoApply:bool=False,styleId:str|None=None,route:Literal['api','chatgpt']='api',resampling:Literal['smooth','nearest']|None=None) -> dict:
    """Start image generation. route api (default) uses the configured API provider; route chatgpt requests native subscription generation through the embedded conversation and returns its connecting/requested status. Project pages automatically include their style, direction and selected characters. Standalone documents accept styleId. edit sends current composite; patch sends surrounding context on a 64-aligned working canvas (at least 1024 on its long side); the selection masks application. resampling nearest preserves hard edges. Returns durable job with assembled prompt and creative context. Sources and candidates are retained. Auto-apply rejects changed documents. Subscription-native generation is available through embedded ChatGPT when its account supports it."""
